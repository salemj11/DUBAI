...
export default function LobbyScreen({ room, lobby, me, testMode, isHost, fillTestPlayers, handleStart }) {
  // Use lobby presence for player list display, fall back to room.players for game state
  const playerNames = lobby.currentCount > 0 ? lobby.players.map((p) => p.name) : room.players;
  const count = Math.max(lobby.currentCount, room.players.length);
  const canStart = count >= MAX_PLAYERS;
  ...
  return (
    <div className="app grain">
      ...
      <div style={{ padding: "40px 24px", textAlign: "center" }}>
        ...
        <p className="fade-up s1" style={{ color: "var(--td)", marginBottom: 32 }}>{count}/{MAX_PLAYERS} players</p>
        ...
        {isHost && (<button onClick={canStart ? handleStart : undefined} style={{ width: "100%", marginTop: 16, padding: 16, borderRadius: 14, background: canStart ? "var(--gold)" : "var(--s)", border: "none", color: canStart ? "#07070c" : "var(--tm)", fontSize: 16, fontWeight: 700, fontFamily: "'Syne',sans-serif", cursor: canStart ? "pointer" : "not-allowed" }}>{canStart ? "Start →" : `Need ${MAX_PLAYERS - count} more`}</button>)}
